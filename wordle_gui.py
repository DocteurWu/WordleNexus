...
class WordleGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Wordle Solver v3.0")
    ...
    def _enable_buttons(self):
        ...
        self.strict_solve_button.config(state=tk.NORMAL)
        self.proba_crisp_button.config(state=tk.NORMAL)
    ...
    def setup_ui(self):
        ...
        ttk.Label(top_frame, text="Wordle Solver Optimized", style="Title.TLabel").pack(
            side=tk.LEFT
        )

        action_buttons_frame = ttk.Frame(top_frame)
        action_buttons_frame.pack(side=tk.RIGHT)
        self.openings_button = ttk.Button(
            action_buttons_frame,
            text="Suggest Openers",
            command=self.suggest_openers,
            state=tk.DISABLED,
        )
    ...
    def setup_personal_solver_ui(self, parent):
        ttk.Label(parent, text="Personal Solver", style="Header.TLabel").pack(
        ...
        word_entry = ttk.Entry(
            input_frame,
            textvariable=self.personal_word_var,
            width=10,
            font=("Courier", 10),
        )
        ...
        for i in range(5):
            btn = tk.Button(
                ...
                width=3,
                bg=self.COLORS["gray"],
                command=lambda i=i: self._cycle_personal_color(i),
            )
        ...
        self.personal_submit_button = ttk.Button(
            action_frame,
            text="Submit Attempt",
            command=self.submit_personal_attempt,
            state=tk.DISABLED,
        )
        self.personal_submit_button.pack(side=tk.LEFT)
        ttk.Button(action_frame, text="Reset", command=self.reset_personal_solver).pack(
            side=tk.LEFT, padx=5
        )
        self.prefilter_community_button = ttk.Button(
            action_frame,
            text="Pre-filter with Community",
            command=self.prefilter_with_community,
        )
        self.prefilter_community_button.pack(side=tk.LEFT, padx=5)
        self.prefilter_live_button = ttk.Button(
            action_frame,
            text="Pre-filter with Live Results",
            command=self.prefilter_with_live_results,
            state=tk.DISABLED,
        )
        ...
        self.personal_info_var = tk.StringVar(
            value="Enter your first guess (e.g., SALET) and the resulting pattern."
        )
        ttk.Label(
            info_frame,
            textvariable=self.personal_info_var,
            wraplength=350,
            justify=tk.LEFT,
        ).pack(anchor=tk.NW)

        ttk.Label(info_frame, text="Suggestions:", style="Header.TLabel").pack(
            anchor=tk.W, pady=(10, 0)
        )
        self.personal_suggestions_text = tk.Text(
            info_frame, height=5, font=("Courier", 10), state=tk.DISABLED, wrap=tk.WORD
        )
        self.personal_suggestions_text.pack(fill=tk.X, pady=5)

        ttk.Label(info_frame, text="Remaining Words:", style="Header.TLabel").pack(
            anchor=tk.W, pady=(10, 0)
        )
        self.personal_remaining_text = tk.Text(
            info_frame, height=10, font=("Courier", 9), state=tk.DISABLED, wrap=tk.WORD
        )
    ...
    def _cycle_personal_color(self, index):
        ...
        new_color_name = (
            "yellow"
            if current_bg == self.COLORS["gray"]
            else "green"
            if current_bg == self.COLORS["yellow"]
            else "gray"
        )
    ...
    def submit_personal_attempt(self):
        ...
        if self.is_strict_analysis_running:
            messagebox.showinfo(
                "Analysis Running",
                "An analysis is currently running. Please stop it before submitting a new attempt.",
            )
            return
        ...
        if len(guess) != 5 or not guess.isalpha():
            messagebox.showerror(
                "Invalid Guess", "Guess must be 5 alphabetic characters."
            )
            return

        pattern = "".join(
            [
                {"#6aaa64": "🟩", "#c9b458": "🟨", "#787c7e": "⬛"}[btn.cget("bg")]
        ...
        self.personal_info_var.set(
            f"Attempt '{guess.upper()}' processed. {remaining_count} candidates remaining."
        )
        ...
        if pattern == "🟩" * 5:
            messagebox.showinfo(
                "Congratulations!", f"You solved it with '{guess.upper()}'!"
            )
    ...
    def setup_community_solver_ui(self, parent):
        ...
        ttk.Label(parent, text="Community Solver", style="Header.TLabel").grid(
        ...
        ttk.Button(controls_frame, text="Add Player", command=self.add_player).pack(
            side=tk.LEFT, padx=5
        )
        self.solve_button = ttk.Button(
            controls_frame,
            text="Lax Analysis",
            command=self.solve_lax_community,
            state=tk.DISABLED,
        )
        self.solve_button.pack(side=tk.LEFT, padx=5)
        self.strict_solve_button = ttk.Button(
            controls_frame,
            text="Exact ERS Analysis",
            command=self.solve_exact_community,
            state=tk.DISABLED,
        )
        self.strict_solve_button.pack(side=tk.LEFT, padx=5)
        self.proba_crisp_button = ttk.Button(
            controls_frame,
            text="Proba Crisp",
            command=self.toggle_proba_crisp,
            state=tk.NORMAL,
        )
        self.proba_crisp_button.pack(side=tk.LEFT, padx=5)
        self.proba_crisp_var = tk.BooleanVar(value=False)
        self.stop_analysis_button = ttk.Button(
            controls_frame,
            text="Stop Analysis",
            command=self.stop_strict_analysis,
            state=tk.DISABLED,
        )
        ...
        # Live Results
        self.live_results_frame = ttk.LabelFrame(
            parent, text="Live Remaining Candidates"
        )
        self.live_results_frame.columnconfigure(0, weight=1)
        self.community_live_results_text = tk.Text(
            self.live_results_frame,
            height=5,
            font=("Courier", 9),
            state=tk.DISABLED,
            wrap=tk.WORD,
        ...
        self.community_live_words_text_widget = tk.Text(
            self.live_words_frame,
            height=5,
            font=("Courier", 9),
            state=tk.DISABLED,
            wrap=tk.WORD,
        )
        self.community_live_words_text_widget.pack(
        ...
        self.community_results_text = tk.Text(
            self.results_frame,
            height=12,
            font=("Courier", 9),
            state=tk.DISABLED,
    ...
    def add_player(self):
        ...
        player_frame = ttk.LabelFrame(
            self.players_frame, text=f"Player {player_idx + 1}"
        )
        ...
        ttk.Button(
            controls_frame,
            text="Add Attempt",
            command=lambda p_idx=player_idx: self.add_community_attempt(p_idx),
        ).pack(side=tk.LEFT)
        ttk.Button(
            controls_frame,
            text="Remove Player",
            command=lambda p_idx=player_idx: self.remove_player(p_idx),
        ).pack(side=tk.LEFT, padx=5)
    ...
    def add_community_attempt(self, player_idx, initial_pattern=None):
        ...
        pattern_entry = ttk.Entry(
            attempt_frame, textvariable=pattern_var, width=10, font=("Courier", 10)
        )
        pattern_entry.pack(side=tk.LEFT, padx=5)
        pattern_entry.bind(
            "<Return>",
            lambda event, p_idx=player_idx: self._handle_community_enter(p_idx),
        )
        ...
        remove_button = ttk.Button(
            ...
            text="×",
            width=2,
            command=lambda p_idx=player_idx,
            a_idx=attempt_idx: self.remove_community_attempt(p_idx, a_idx),
        )
        remove_button.pack(side=tk.LEFT, padx=5)

        player_data["attempts"].append(
            {
                "frame": attempt_frame,
    ...
    def remove_community_attempt(self, player_idx, attempt_idx):
        ...
        if len(player_data["attempts"]) > 1:  # Always keep at least one attempt row
            ...
            for i, att in enumerate(player_data["attempts"]):
                ...
                att["remove_button"].config(
                    command=lambda p_idx=player_idx,
                    a_idx=i: self.remove_community_attempt(p_idx, a_idx)
                )
    ...
    def _handle_community_enter(self, player_idx):
        ...
        if player_data["attempts"]:
            last_pattern = (
                player_data["attempts"][-1]["pattern_var"].get().strip().upper()
            )
            ...
            if parsed_pattern == GREEN * 5:
                ...
                self.root.after(
                    100,
                    lambda: self.players_data[player_idx + 1]["attempts"][0]["frame"]
                    .winfo_children()[1]
                    .focus_set(),
                )
                return
        ...
        self.root.after(
            100,
            lambda: player_data["attempts"][-1]["frame"]
            .winfo_children()[1]
            .focus_set(),
        )
    ...
    def solve_lax_community(self):
        ...
        threading.Thread(
            target=self._run_solve_thread,
            args=(self.last_players_grids, [], "lax"),
            daemon=True,
        ).start()

    def solve_exact_community(self):
        """Run Exact ERS Analysis directly on lax candidates"""
        if not self.solver or not self.lax_candidates:
            messagebox.showwarning(
                "No Lax Candidates",
                "Please run Lax Analysis first to generate candidates.",
            )
            return
        self.status_var.set("Running Exact ERS Analysis...")
        self.solve_button.config(state=tk.DISABLED)
        self.strict_solve_button.config(state=tk.DISABLED)
        self.proba_crisp_button.config(state=tk.DISABLED)
        threading.Thread(
            target=self._run_exact_ers_thread,
            daemon=True,
        ).start()

    def _run_exact_ers_thread(self):
        """Compute exact ERS on lax candidates without re-filtering"""
        try:
            start = time.time()
            candidates = self.lax_candidates
            best = self.solver.exact_solver.get_best_guesses(candidates, top_n=20)
            ...
            # ====== PROBA CRISP LOGIC ======
            if self.proba_crisp_var.get():
                # Use letter frequency scoring for probabilities
                from solver import config

                scores = {}
                total_score = 0
                for w in candidates:
                    score = sum(config.letter_freq.get(c, 0.1) for c in w)
                    scores[w] = score
                    total_score += score
                probs = {w: (s / total_score) * 100 for w, s in scores.items()}
            else:
                # Uniform probability
                prob_val = 100.0 / total_candidates if total_candidates > 0 else 0
                probs = {w: prob_val for w in candidates}

            # Build candidate table with individual ERS
            cand_lines = [
                f"{'CANDIDAT':<12}{'PROBA':<10}{'ERS':<10}",
                "-" * 32,
            ]
            # Get ERS for each candidate
            cand_with_ers = self.solver.exact_solver.get_all_ers(candidates)

            for w, ers in cand_with_ers:
                cand_lines.append(f"{w.upper():<12}{probs[w]:.1f}%     {ers:.2f}")

            # Build best guesses table (top 20 from all 14k)
            guess_lines = [
                "",
                f"{'RANK':<6}{'GUESS':<10}{'ERS':<10}{'TYPE'}",
                "-" * 40,
            ]
            for i, (word, ers) in enumerate(best, 1):
                typ = "CANDIDAT" if word in candidates else "guess"
                guess_lines.append(f"{i:<6}{word.upper():<10}{ers:.2f}     {typ}")

            # ====== LOGIQUE "UNE PIERRE DEUX COUPS" ======
            best_guess_word = best[0][0]
            best_guess_ers = best[0][1]
            best_candidate = None
            best_candidate_ers = float("inf")
            for w, ers in cand_with_ers:
                if ers <= best_guess_ers and ers < best_candidate_ers:
                    best_candidate = w
                    best_candidate_ers = ers

            if best_candidate:
                final_recommendation = best_candidate.upper()
                final_type = "CANDIDAT"
                final_ers = best_candidate_ers
                bonus_msg = "  [UNE PIERRE DEUX COUPS: jouer ce mot trouve LE mot en 1 coup si c'est lui]"
            else:
                final_recommendation = best_guess_word.upper()
                final_type = "GUESS"
                final_ers = best_guess_ers
                bonus_msg = ""
            ...
            lines = [
                ...
                "EXACT ERS ANALYSIS",
                "=" * 80,
                f"Candidates: {total_candidates} | Time: {elapsed:.2f}s",
                f"Proba Mode: {'Crisp' if self.proba_crisp_var.get() else 'Uniform'}",
                "",
                *cand_lines,
                *guess_lines,
                "",
                "=" * 80,
                f"MEILLEUR COUP: {final_recommendation} (ERS={final_ers:.2f}) [{final_type}]{bonus_msg}",
                "=" * 80,
            ]
            ...
            self.root.after(
                ...
                lambda: self._update_text_widget(
                    self.community_results_text, results_text
                ),
            )
            self.status_var.set("Exact ERS Analysis complete.")

        except Exception as e:
            logger.error(f"Error in exact ERS thread: {e}", exc_info=True)
            self.root.after(
                0,
                lambda: messagebox.showerror(
                    "Error", f"An error occurred during exact ERS analysis:\n{e}"
                ),
            )
            self.status_var.set("Exact ERS Analysis failed.")
        finally:
            ...
            self.root.after(0, lambda: self.proba_crisp_button.config(state=tk.NORMAL))

    def toggle_proba_crisp(self):
        """Toggle proba crisp mode"""
        self.proba_crisp_var.set(not self.proba_crisp_var.get())
        state = "ON" if self.proba_crisp_var.get() else "OFF"
        self.status_var.set(f"Proba Crisp: {state}")
        # Change button text/color to show state
        if self.proba_crisp_var.get():
            self.proba_crisp_button.config(
                text="Proba Crisp [ON]", style="Accent.TButton"
            )
        else:
            self.proba_crisp_button.config(text="Proba Crisp", style="TButton")

    def solve_probabilistic_community(self):
        if not self.solver or not self.lax_candidates:
            messagebox.showwarning(
                "No Lax Candidates", "Please run Lax Analysis first."
            )
            return
        self.live_results_frame.grid(
        ...
        self.status_var.set("Running Probabilistic Analysis...")
        self.solve_button.config(state=tk.DISABLED)
        self.strict_solve_button.config(state=tk.DISABLED)
        self.proba_crisp_button.config(state=tk.DISABLED)
        ...
        threading.Thread(
            ...
            args=(
                self.last_players_grids,
                self.personal_solver.attempts,
                "probabilistic",
                self.personal_solver.remaining_candidates,
            ),
            daemon=True,
        ).start()
    ...
    def _get_players_grids(self):
        ...
        for p_data in self.players_data:
            grid = [
                att["pattern_var"].get().strip().upper()
                for att in p_data["attempts"]
                if att["pattern_var"].get().strip()
            ]
            if grid:
                if any(len(p) != 5 for p in grid):
                    messagebox.showerror(
                        "Invalid Pattern",
                        f"A pattern for Player {self.players_data.index(p_data) + 1} is not 5 characters long.",
                    )
                    return None
                players_grids.append(
                    self.personal_solver.parse_pattern_input("\n".join(grid)).split(
                        "\n"
                    )
                )
        return players_grids

    def _run_solve_thread(
        self,
        players_grids,
        personal_attempts,
        validation_mode="lax",
        ...
        self.live_remaining_candidates = (
            sorted(list(candidates_to_check)) if candidates_to_check else []
        )
        is_deep = validation_mode == "probabilistic"
        if is_deep:
            self.is_strict_analysis_running = True
            self.stop_strict_analysis_event.clear()
        try:
            self.root.after(
                0,
                lambda: self._update_text_widget(self.community_live_results_text, ""),
            )
            self.root.after(
                0, lambda: self._update_text_widget(self.community_results_text, "")
            )
            self.root.after(
                0,
                lambda: self._update_text_widget(
                    self.community_live_words_text_widget, ""
                ),
            )

            mode_label = validation_mode.capitalize()

            _ui_batch_counter = 0
            _UI_BATCH_SIZE = 25
            _max_display_words = 150

            def _flush_live_results():
                count = len(self.live_remaining_candidates)
                display = self.live_remaining_candidates[:_max_display_words]
                text = ", ".join(w.upper() for w in display)
                if count > _max_display_words:
                    text += f" ... ({count} total)"
                self.root.after(
                    0,
                    lambda t=text: self._update_text_widget(
                        self.community_live_results_text, t
                    ),
                )

            def progress_handler(update):
                nonlocal _ui_batch_counter

                if self.stop_strict_analysis_event.is_set():
                    raise InterruptedError("Analysis stopped by user.")

                if update["type"] == "player_start":
                    status_msg = f"{mode_label} (Player {update['player_idx']}/{update['num_players']}): {update['num_candidates']} words..."
                    ...
                    status_msg = f"{mode_label} (Player {update['player_idx']}): {update['current']}/{update['total']} ({update['pct']:.1f}%) | ETA: {eta_str}"
                    self.root.after(0, lambda: self.status_var.set(status_msg))
                    _flush_live_results()
                    ...
                    self.root.after(
                        0,
                        lambda: self._update_text_widget(
                            self.community_live_results_text, live_text
                        ),
                    )
                elif update["type"] == "word_validated":
                    self.root.after(
                        0,
                        lambda w=update["word"]: self._update_text_widget(
                            self.community_live_words_text_widget,
                            f"Checking: {w.upper()}",
                        ),
                    )
                    if not update["is_valid"]:
                        if update["word"] in self.live_remaining_candidates:
                            ...
                            _ui_batch_counter += 1
                            if _ui_batch_counter >= _UI_BATCH_SIZE:
                                _ui_batch_counter = 0
                                _flush_live_results()

            results, candidates = self.solver.solve(
                players_grids,
                personal_attempts,
                validation_mode=validation_mode,
                candidates_to_check=candidates_to_check,
                progress_callback=progress_handler,
                stop_event=self.stop_strict_analysis_event,
            )
            ...
            if validation_mode == "lax":
                ...
                self.root.after(
                    0, lambda: self.strict_solve_button.config(state=tk.NORMAL)
                )
                self.root.after(
                    0, lambda: self.proba_crisp_button.config(state=tk.NORMAL)
                )
            else:
            ...
            self.root.after(
                0,
                lambda: self._update_text_widget(self.community_results_text, results),
            )
            ...
            if is_deep:
                self.root.after(
                    0,
                    lambda: self._update_text_widget(
                        self.community_live_results_text,
                        f"All {mode_label.lower()} validations passed.",
                    ),
                )
            ...
            self.root.after(
                0, lambda: self.prefilter_live_button.config(state=tk.DISABLED)
            )
            self.root.after(
                0, lambda: self.stop_analysis_button.config(state=tk.DISABLED)
            )
    ...
    def suggest_openers(self):
        ...
        result_str = "\n".join(
            [
                f"{i:<2}. {word.upper():<8} (ERS: {score:.2f})"
                for i, (word, score) in enumerate(openers, 1)
            ]
        )
    ...
    def prefilter_with_community(self):
        ...
        if self.strict_candidates:
            ...
            messagebox.showwarning(
                "No Community Data",
                "Please run a 'Lax Analysis', 'Moderate Analysis', or 'Probabilistic Analysis' on the community data first to generate a candidate list.",
            )
            return
        ...
        self.personal_info_var.set(
            f"Prefiltered with community results. {remaining_count} candidates remaining."
        )
        ...
        messagebox.showinfo(
            "Prefilter Complete",
            f"Personal solver's word list has been updated with the {remaining_count} candidates from the community's analysis.",
        )

    def prefilter_with_live_results(self):
        if not self.personal_solver or not self.live_remaining_candidates:
            messagebox.showwarning(
                "No Live Data", "No live results available to prefilter."
            )
            return
        ...
        self.personal_info_var.set(
            f"Prefiltered with live results. {remaining_count} candidates remaining."
        )
        ...
        messagebox.showinfo(
            "Prefilter Complete",
            f"Personal solver's word list has been updated with the {remaining_count} live candidates.",
        )
